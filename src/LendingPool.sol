...
contract LendingPool is ReentrancyGuard {
    ...
    function liquidate(address borrower) external nonReentrant {
        ...
        IERC20(address(mxt)).safeTransferFrom(msg.sender, address(this), debt);
        mxt.burn(debt);
    ...
    function _collateralValue(address user) internal view returns (uint256) {
        Position memory pos = positions[user];
        // ethPrice: 18-dec USD per 1 ETH — collateral: 18-dec ETH — result: 18-dec USD
    ...
    function _healthFactor(address user) internal view returns (uint256) {
        ...
        uint256 collUSD = _collateralValue(user);
        // (collUSD * LIQUIDATION_THRESHOLD%) / debtUSD
        // dividing by 100 converts LIQUIDATION_THRESHOLD from percentage to fraction
